...
import getopt
import sys
from os import getcwd, listdir, path, sep, makedirs
import matplotlib
import numpy as np
np.seterr(all="raise")
import pandas as pd
...
def parse_stats_from_runs(experiment_name, gen_no=1000, skip=False):
    ...
    for stat in [stat for stat in stats if stat not in no_parse_list and
        ...
        try:
            ...
            np.savetxt(path.join(file_path + "-results", str(gen_no), (stat + ".csv")), summary_stats,
                       delimiter=",")
    
            # Graph stat by calling graphing function.
            save_average_plot_across_runs(path.join(file_path + "-results", str(gen_no), (stat + ".csv")))
    ...
    for run in runs:
        ...
        try:
            ...
            for i in range(gen_num):
                data_local.append(
                    (8 * alignment[i] + 2 * generalization[i] + 4 * precision[i] + 2 * simplicity[i]) / 16)
            summary_stats.append(data_local[:gen_num])
    ...
    np.savetxt(path.join(file_path + "-results", str(gen_no), "full_stats.csv"), full_stats,
...
def multi_save_average_plot_across_runs(filename, filename2):
    """
    Saves an average plot of multiple runs. Input file data must be of the
    format:
...
if __name__ == "__main__":
    ...
    parse_stats_from_runs(sys.argv[1], sys.argv[2], sys.argv[3] == 'True')
