...
class GateTest(unittest.TestCase):
    ...
    def test_get_next_possible_states(self):
        ...
        actual = set(list(events_with_parents[e3].get_next_possible_states((e1, e2, e3), e3, e4)))
    ...
    def test_get_next_possible_states_9_3(self):
        gate = SeqGate()
        gate.parse('seq({a})xor({b}{f}{c}){d}and(xor({h}xor({c}lo0(xor({b}{f}{d}{g})))){e})opt({h})')
    ...
    def test_get_next_possible_states_9_4(self):
        gate = SeqGate()
        gate.parse('seq({a})xor({b}{f}{c}){d}and(xor({h}xor(and(xor(and({c})))lo0(xor({b}{f}{d}{g})))){e})opt({h})')
    ...
    def test_get_next_possible_states_9_5(self):
        ...
        actual = set(list(events_with_parents[e2].get_next_possible_states((e1, e2), e2, e3)))
        self.assertEqual(2, len(actual))

    def test_get_next_possible_states_9_6(self):
        gate = SeqGate()
        gate.parse('and(xor({d}{a})and({d}{c}))xor(lop({g})seq({e}{f}{d}{b}))and(opt({h}){e})')
    ...
    def test_get_next_possible_states_9_7(self):
        gate = SeqGate()
        gate.parse('{a}and(and(opt({e})seq(xor({d}{e}){b}))xor(opt({g})xor(xor({h}xor({f}{h}))xor({f}and(and({c}{h}){a})))))')
    ...
    def test_get_next_possible_states_9_8(self):
        gate = SeqGate()
        gate.parse('{a}xor({f}{b})opt(lo1(and({g}xor({f}{b})){d}){c}{d})and(seq({e}))')
        ...
        actual = set(list(events_with_parents[e8].get_next_possible_states((e1, e2, e3, e4, e5, e6, e7, e8), e8, e9)))
        self.assertEqual(2, len(actual))
