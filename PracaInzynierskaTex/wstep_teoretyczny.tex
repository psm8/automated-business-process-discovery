...
Wśród istniejących algorytmów mogą pojawić się problemy ze współbieżnością, pomijaniem aktywności, możliwością reprezentowania duplikatów, nie radzi sobie z zakłócenia w logu, nie bieże pod uwagę częstotliwości wystąpień danego wariantu, tworzy zbyt skomplikowane modele, nie radzi sobie z odwzorowaniem niektórych zachowań np. krótkich pętli, może tworzyć model ze zła strukturą Oparte na directly follows graphs przez co problemem może być kiedy log jest niekompletny 
...
Algorytmy genetyczne do automatycznego odkrywania procesów biznesowych mogą być odpowiedzią na te problemy. Takie podejście pozwala na wyeliminowanie część problemów często dotyczących innych metod.
...
POET
Ideą jest promowanie rozwiązywania prostych problemów w prosty sposób \newline
...
Unikanie niewystarczającego dopasowania(\textit{eng. underfitting}) \newline
...
W pierwszej chwili może wydwać się przeciwieństwem precyzji, dobrym przykładem żeby to zwizualizować jest plama albo to z przetwarzania obrazów, chcemy wypełnić zagłębienia jednocześnie nie pozwalając na rozszerzenie. \newline
...
W sekcji \ref{sec:event_logs} przedstawiono przykład dziennika zdarzeń i warianty procesu obliczone dla niego. Weźmy pod uwagę model aandd

Obliczanie metryk dla tego modelu