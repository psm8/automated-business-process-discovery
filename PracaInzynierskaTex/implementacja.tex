...
Odwzorowanie obliczane jest osobno dla każdego wariantu w logu. Pozostałe metryki są obliczane dla najlepiej dopasowanej gramatyki. Jeśli błąd odwzorowania wynosi 0, to ilość wystąpień danego wariantu konieczny do obliczenia precyzji jest zapisywany w słowniku. Dodawana do każdej aktywności jest też ilość jej dotychczasowych wystąpień potrzebna we wzorze na generalizację.
...
def oblicz_metryki(log_info, model, najkrótsza_dozwolona_długość, 
   ...
   for wariant in log:
      najlepszy_błąd_lokalny, najlepiej_dopasowana_ścieżka, najlepszy_wariant := 
      ...
      if najlepszy_błąd_lokalny == 0:
         idealnie_dopasowane_logi[najlepiej_dopasowana_ścieżka] := 
         log_info[wariant].ilość_wystąpień
      dodaj_wystąpienia(lista_zdarzeń_w_modelu, najlepiej_dopasowana_ścieżka, 
                        log_info[wariant].ilość_wystąpień)
...
Mając już sparsowany model, kolejnym krokiem jest obliczenie metryk. Najbardziej problematyczną metryką do obliczenia jest dopasowanie. Całą procedurę można podzielić następująco:
\begin{itemize}
...
\lstset{label=src:best_result, frame=single}
\begin{lstlisting}
def oblicz_odwzorowanie_dla_jednego_wariantu(proces, model, najkrótsza_dozwolona_długość, 
				               najdłuższa_dozwolona_długość, cache):
