...
Argumentami metody model jako obiekt Gate oraz wariant jako tablica znakow. Zwracane sa natomiast lista
...
def oblicz_dopasowanie(model, wariant):
   ...
   n = długość(wariant) + 1  # Macierz rozwiązań ilość kolumn.
   ...
   for i in range(1, m):
      if should_go_recurrent(model[i-1]):
         ...
         dopasowanie(macierz_rozwiazań, model[i - 1], wariant, kara, i, n)

   ścieżka := znajdź_ściezkę(macierz_rozwiązań, błąd['PRZERWA'], model, wariant,                
...
Argumentami metody długość szukanej ścieżki oraz wariant potrzebny dla obiektu LopGate. Zwracana jest natomiast lista wszystkich ścieżek o określonej długość jako obiekty BaseGroup.

\lstset{caption=Znajdowanie ścieżki w modelu, captionpos=b}
\lstset{label=src:traceback, frame=single}
\begin{lstlisting}[escapeinside=``]
def znajdź_sciezkę(macierz_rozwiązań, model, wariant, rozwiązania_podmodeli) -> [Event]:
   sciezka = []
   while i != 0:
      ...
      if model_results_local[i] is not None:
         ...
         if macierz_rozwiązań[i][j] == 
            macierz_rozwiązań[i - 1][j] + długość_grupy_zdarzeń * błąd['PRZERWA']:
