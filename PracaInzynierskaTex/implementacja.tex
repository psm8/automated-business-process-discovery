...
Projektując gramatykę procesu biznesowego przyjęto dwa początkowe założenia. Założono, że generowane modele muszą być łatwe do przełożenia na notację BPMN oraz nie powinny być tworzone modele niespójne strukturalnie, co pozwali na zredukowanie przestrzeni rozwiązań, jednocześnie tylko tworząc rozwiązania na pewno nie generujące błędów.

Przy tworzeniu gramatyki procesu biznesowego ważne jest, żeby znaleźć balans, jeśli chodzi o poziom skomplikowania modelu jaki będzie możliwy do wygenerowania używając zaproponowanej gramatyki.  W pracy \cite{10.1007/978-3-540-69534-9_35} autorzy przeanalizowali składniki języka BPMN pod kątem częstotliwości ich stosowania. Najczęściej stosowanymi elementami modelów procesu biznesowego, jeśli chodzi o bramki są: XOR - ALBO kodowane w gramatyce jako xor, AND - I jako and oraz pętle jako lo<0$\_$n>. Do przedstawionej poniżej gramatyki dodano także bramkę OR - LUB  jako opt, czyli jako uogólnienie bramki xor w celu uniknięcia zagnieżdżonych bramek xor. Ponadto koniecznym jest symbolu seq, która oznacza normalny przepływ procesów.

Zdarzenia początkowe i końcowe nie muszą być reprezentowane w gramatyce, gdyż zgodnie z zaleceniami w sekcji \ref{sec:modelling} przyjmuje się, że dobrą praktyką jest żeby model zawierał tylko jedno zdarzenie początkowe i końcowe. Z tego powodu, przyjęto, że zdarzenia początkowe i końcowe są domyślnie na początku i końcu wygenerowanego słowa. 

Zgodnie z opisanym w sekcji \ref{grammarCreation} problemem dotyczącymi tworzenia gramatyki dla metod opartych o krzyżowanie i mutację punktową lub n-punktową, zdecydowano się na stworzenia gramatyki pod kątem operacji stosujących te operacje na drzewie, czyli fenotypie. Często słabym punktem algorytmów ewolucyjnych staje się lokalne przeszukiwanie. Przechowując fenotyp jako drzewo i stosując wspomniane operatory, prawdopodobna jest sytuacja, że mała modyfikacja blisko korzenia drzewa może poprawić rozwiązanie, jednak jej zaistnienie przy użyciu tych operatów wymaga wygenerowania identycznego poddrzewa na nowo, co sprawia, że prawdopodobieństwo zajścia takiej operacji jest niskie. Do rozwiązania tego problemu mogą służyć metody inspirowane algorytmami memetycznymi oparte, a o reprezentacje jako drzewo\cite{memetic}, których główną ideą jest aplikowanie lokalnych zmian na drzewie bez konieczności regenerowania całego poddrzewa, co pozwala na usprawnienie procesu ewolucji. Niestety, użyta biblioteka nie posiada metod pozwalających na lokalne przeszukiwanie, dlatego żeby w pewnym stopniu zaradzić temu problemowi, gramatyka stara się zmniejszać głębokość drzewa, jednocześnie zwiększać szanse na lokalne mutacje poprzez wprowadzenie symbolu nieterminalnego <slots>, których sprawia, że drzewo rośnie bardziej wszerz i posiada oprócz jednego symbolu <anygate>, którego użycie ma zapewnić tworzenie poprawnych, niepustych rozwiązań, symbole <slot>, które dają tylko 10\% szans na produkcje wstawiającą symbol <anygate>. Pozwala to na lokalne zzmiany na późniejszych etapach ewolucji. Zaadresowanie to także poprzez reprezentowanie bramki jako <name>(<slots>), dzięki czemu w wyprowadzeniu nazwa bramki <name> jest oddzielona on jej zawartości, co sprawia, że możliwa jest zmiana nazwy bramki blisko korzenia drzewa bez modyfikacji jej wnętrza.

Zaadresowano także konieczność częstotliwość występowania poszczególnych bramek logicznych . Zgodnie z \cite{10.1007/978-3-540-69534-9_35} normalne połączenie i bramki XOR - ALBO, AND - I są tworzone przez większą ilość produkcji niż rzadziej występujące pętle i bramki OR - LUB. 
...
Model dla którego zaprezenentowanie obliczanie metryk (rys. \ref{fig:metrics_business_process})byłby za pomocą powyższej notacji zakodowany jako:
\begin{center}
(\{a\}and(\{b\}\{c\})opt(\{b\}\{e\})\{d\}
\end{center}

Zapis lo<0$\_$n> jest nieoczywisty, jednak konieczny do zakodowania pętli, które są kończone na innej aktywności niż pełna ich iteracja.  
...
\begin{center}
\{a\}and(xor(\{b\}\{c\})\{d\}lop(\{e\}and(xor(\{b\}\{c\})\{d\})xor(\{f\}\{g\})
\end{center}
Użycie powyższej notacji jest poprawne, jednak kodowanie pętli w tej sposób sprawia, że powstałe słowo jest skomplikowane, a jego wyewoluowanie mało prawdopodobnie - znacznie mniej prawdopodobne niż wyewoluowanie pętli klasycznych pętli, problem ten rozwiązano używając lo<0$\_$n>, gdzie <0$\_$n> oznacza ile ostatnich znaków nie powinno być uwzględniane w pierwszej iteracji pętli, dzięki czemu możliwe jest zakodowanie pętli, które są kończone na innej aktywności niż pełna ich iteracja, a jednocześnie potrzeba do tego znacznie mniejszej liczb symboli, co sprawia, że wyewoluowanie takiego modelu jest znacznie bardziej prawdopodobne.
...
\begin{figure}[h]
	\centering{\includegraphics[scale=0.4]{GateUML.png}}
...
\begin{figure}[h]
	\centering{\includegraphics[scale=0.4]{EventUML.png}}
...
\subsubsection{Pozostałe wnioski dotyczące implementacji}
Cache
